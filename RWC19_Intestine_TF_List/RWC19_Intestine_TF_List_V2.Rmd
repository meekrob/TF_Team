...
wTF3 <- read.delim("../../10_DATASETS/05_TF3_0/TF3-0_namesonly.txt", na.strings = c("", "NA"))
...
onish_adult <- read.csv("../../10_DATASETS/18_Hand_Dissected_Adult_Intestine_RNA-seq/171214_res_gutsVwt_DESeq.csv")
...
tintori_1E <- read.csv("../../10_DATASETS/17_Tintori_et_al_2016_Embryo_scRNA-seq/8cell_vs_1E_p1_00.csv")
...
tintori_2Ep <- read.csv("../../10_DATASETS/17_Tintori_et_al_2016_Embryo_scRNA-seq/16cell_vs_Ep_p1_00.csv")
...
hashimshony_embryo <- read.csv("../../10_DATASETS/12_Hashimshony_et_al_2015_CEL-seq/Hashimshony_Endoderm_Genes.csv", header = TRUE)
...
blazieGenes <- read_excel("../../10_DATASETS/13_Blazie_et_al_2015_PAB-1_Pulldown_RNA-seq/Blazie_et_al_2015_Table_S3_ges-1_all_genes.xlsx")
blazieGenesSubset <- data.frame(blazie_WBGeneID = blazieGenes[1], blazie_FPKM = blazieGenes[8])
colnames(blazieGenesSubset) <- c("blazie_WBGeneID", "blazie_FPKM")

#View(blaizeGenesSubset)
int_TF_df <- merge(x = int_TF_df, y = blazieGenesSubset, by.x = "WBGeneID", by.y = "blazie_WBGeneID", all.x = TRUE)

# add boolian column
int_TF_df <- int_TF_df %>% mutate(blazie_bool = if_else(int_TF_df$WBGeneID %in% blazieGenesSubset$blazie_WBGeneID, TRUE, FALSE))
...
haenniGenes <- read.csv("../../10_DATASETS/14_Haenni_et_al_Intestine_Nuclei_3end-seq/gks282_supplementary_data/nar-00455-f-2012-File008.csv", header = TRUE)
...
colnames(haenniGenes) <- str_c("haenni_FANS_", colnames(haenniGenes))
haenni_subset <- data.frame(haenniGenes[1], haenniGenes[6], haenniGenes[7], haenniGenes[8], haenniGenes[9], haenniGenes[10])

#add to master dataframe
int_TF_df <- merge(x = int_TF_df, y = haenni_subset, by.x = "Sequence.name", by.y = "haenni_FANS_gene", all.x = TRUE)

# compute boolian present/absent column based on authors grouping
haenni_subset_intestine <- haenni_subset %>% filter(haenni_FANS_group == "Sorted")
int_TF_df <- int_TF_df%>% mutate(haenni_FANS_bool = if_else(int_TF_df$Sequence.name %in% haenni_subset_intestine$haenni_FANS_gene, TRUE, FALSE))
...
pauliGenes <- read_excel("../../10_DATASETS/15_Pauli_et_al_2006_PAB-1_Pulldown_microarray/SupTable1_intestine_enriched_genes.xlsx")
...
spencerLEgenes <- read.table("../../10_DATASETS/16_Spencer_et_al_2010_FACS_and_pulldown_tilling_array/LE-intestine_enr_vs_ref.WS200.txt", quote="\"", comment.char="", header = TRUE)
...
spencerL2genes <- read.table("../../10_DATASETS/16_Spencer_et_al_2010_FACS_and_pulldown_tilling_array/L2-intestine_enr_vs_ref.WS200.txt", quote="\"", comment.char="", header = TRUE)
...
```{r}
ELT2_EE_LE_L3_Peaks <- read.delim("./valerie_peaks_processed.bedlike")
...
```{r}
write.csv(int_TF_df, "./intestine_TF_database.csv")
write.csv(RWC19_Metadata, "./intestine_TF_metadata.csv")
```
...
clustMatrix <- int_TF_df %>% select(hashimshony_embryo_bool, pauli_L4_bool, spencer_LE_bool, spencer_L2_bool, tintori_1E_bool, tintori_2Ep_bool, onish_adult_bool, haenni_FANS_bool, blazie_bool, ELT2_peak_k1_bool, ELT2_peak_k2_bool, ELT2_peak_k3_bool, ELT2_peak_k4_bool, ELT2_peak_k_na_bool)
